...
faq:
  ...
  - q: "What's s.jina.ai?"
    a: "Search that reads: it runs your query, then fetches and fully extracts the top results (up to 20, set with the num parameter) through the same Reader stack — so instead of titles and snippets, your agent gets the actual content of the best pages in one call. It requires an API key."
...
- **Real-web handling** — headless Chrome for JS-heavy pages, or a faster plain-HTTP fetch via `x-engine: direct`; PDFs via PDF.js and Word/Excel/PowerPoint via LibreOffice, including direct binary upload.
- **Vision built in** — optional VLM captions (`x-with-generated-alt: true`) fill in missing alt text, so text-only models don't lose the figures.
...
- **`s.jina.ai`** — query in, *full content* of the top results out (up to 20, set with `num`): search and read collapsed into one call.

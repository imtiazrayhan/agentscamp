...
faq:
  ...
  - q: "Does a skill uploaded to claude.ai show up in Claude Code?"
    a: "Not for local Claude Code sessions. Claude Code on your machine reads the project's .claude/skills/ and your ~/.claude/skills/, so put the same folder at .claude/skills/<name>/SKILL.md inside the project, or at ~/.claude/skills/<name>/ to make it available everywhere. Cowork and Claude Code cloud sessions are the exception: they load the skills enabled on your claude.ai account, so the upload covers them. Claude loads a skill when a task matches, or you invoke it as /name."

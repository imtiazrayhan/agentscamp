...
featured: false
summary: "Ecosystem and ownership decide it. LangSmith is the first-party choice for LangChain/LangGraph stacks — deepest integration, polished evals, managed SaaS. Langfuse is the open-source, framework-neutral choice — MIT-licensed core, self-hostable for data control, OpenTelemetry-based. Heavy LangChain shops pick LangSmith; everyone else's default tilts Langfuse."
keyTakeaways:
  ...
  - "Langfuse's moat is openness — open-source core, true self-hosting (your data, your infra), framework neutrality via OpenTelemetry-based SDKs and a native OTLP endpoint."
  ...
  - "Pricing shapes: both have free tiers and paid SaaS plans with usage billing (LangSmith's also charge per seat); Langfuse adds the self-host escape hatch that caps spend at infra cost."
faq:
...
sources:
  ...
  - title: "Langfuse OpenTelemetry integration"
    ...
    publisher: "Langfuse"
  - title: "Trace with OpenTelemetry (LangSmith docs)"
...
Once an LLM feature ships, the questions change: *what did the model actually do, why did this trace cost four dollars, which prompt version regressed?* Answering them is observability, and **Langfuse vs LangSmith** is the category's defining matchup — first-party ecosystem depth versus open-source neutrality.
...
**Langfuse** is the open-source engineering platform for the same job: MIT-core tracing, prompt management with versioning and deployment, eval pipelines (LLM-as-judge, human annotation, datasets), and analytics — framework-agnostic by design, with OpenTelemetry-based SDKs (Python/JS), a native OTLP endpoint, and integrations across the gateway/framework landscape. Self-hosting is first-class, not an enterprise afterthought: your traces, your Postgres/ClickHouse, your compliance story. ClickHouse, Inc. acquired Langfuse in January 2026; Langfuse says it stays open source and self-hostable, with no licensing changes planned. [Tool profile →](/tools/langfuse)

## Dimension by dimension

| | Langfuse | LangSmith |
| --- | --- | --- |
| Source/ownership | Open source (MIT core; `ee/` commercial), owned by ClickHouse since January 2026, self-host first-class | Proprietary SaaS (enterprise self-host) |
| Framework fit | Neutral (OTel-based SDKs, native OTLP ingest) | LangChain/LangGraph native; others via SDK or native OTLP ingest |
...
| Cost shape | Free OSS + usage SaaS | Free tier + per-seat plans with usage billing |
...
Whichever you pick, the observability platform is the *substrate* — the value comes from the [eval discipline you run on it](/guides/evaluation/write-llm-evals) and the [production tracing habits](/agents/data-ai/llm-observability-engineer) that catch regressions before users do. The wider tool field (Phoenix, Braintrust, promptfoo, and Helicone, in maintenance mode since March 2026) is mapped in [Best LLM & RAG Evaluation Tools in 2026](/guides/evaluation/best-llm-eval-tools-2026).

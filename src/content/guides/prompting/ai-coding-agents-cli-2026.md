...
seoTitle: "Open-Source & CLI AI Coding Agents 2026: OpenCode, Cline, Aider, Codex"
...
featured: false
summary: "Open-source and CLI coding agents trade polish for control: bring your own model (or run one locally), keep your code on your terms, and script it into CI. OpenCode is the most-starred breakout. Cline and Kilo Code live in VS Code; OpenCode, Aider, and Codex CLI live in the terminal. Choose by where you work and how much you value model and data control."
keyTakeaways:
  - "Open-source agents default to bring-your-own-model: you supply API keys or run a local model, so cost and data handling stay yours (some, like Cline, now also sell inference)."
  - "Cline and Kilo Code are the VS Code extension picks (Roo Code shut down in May 2026; Continue joined Cursor and its repo is read-only); OpenCode, Aider, and Codex CLI are terminal-native; Goose runs locally as a CLI/desktop agent."
...
- **[Roo Code](/tools/roo-code)** — a Cline fork built around **customizable modes** (code, architect, ask, debug), each with its own behavior and tools. **Sunset (May 2026):** the maintainers archived the project on May 15, 2026 and now point users to Cline and ZooCode, a community fork, while Kilo Code publishes a Roo migration guide. Reach for Cline or [Kilo Code](/tools/kilo-code) instead.
...
For a head-to-head of these extensions, including where Roo Code users should go now, see [Cline vs Kilo Code vs Roo Code](/guides/comparisons/cline-vs-kilo-code-vs-roo-code).
...
- **[Goose](/tools/goose)** — an open-source, extensible agent that runs **locally** (CLI and desktop), BYO-model and MCP-first, aimed at developers who want an on-machine autonomous agent. Block contributed it to the Agentic AI Foundation as a founding project (announced December 2025) and completed the move in April 2026; the repo now lives at `aaif-goose/goose`.
...
- [Amp](/tools/amp) — An agentic coding tool built for frontier-model coding, independent since it spun out of Sourcegraph in December 2025.
- [Kilo Code](/tools/kilo-code) — Open-source (MIT) AI coding agent for VS Code that began as a Roo Code fork and was rebuilt on OpenCode in April 2026.
